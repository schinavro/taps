...
from taps.utils.utils import dataframe2dct, dflt
from taps.build import row2paths, read_csv
from taps.utils.shortcut import isstr, isint, isflt, isbool, isDct, asst, isLst
...
class Gaussian(Model):
    ...
    def __setattr__(self, key, value):
        if key[0] == '_':
            ...
            raise AttributeError('key `%s`not exist!')
    ...
    def regression(self, paths, likelihood_type=None, dx=1e-2,
        ...
        X : atomsdata['X']; position of atoms, (D x N) x m dimension array
        Y : atomsdata['Y']; energy and forces of atoms One dimensional array
            with length (m + m x (D x N)) Potential comes first.
        m : mean function
        M : a number of data
        """
        M = getattr(self, 'data_ids', None)
        if M is None or len(M.get('atoms', [])) == 0:
            return self.hyperparameters
        k, m = self.kernel, self.mean
        m.type = self.mean_type
    ...
    def get_data(self, paths, data_ids=None,
                 keys=['positions', 'potential', 'forces']):
        """
        data_ids : dictionary of lists
            {'atoms': [...], 'descriptor': [...]}
        for each key, return
         'positions' -> 'X'; D x M x P
         'potential'    -> 'V'; P
         'forces'    -> 'F'; D x M x P
        ...
        if data_ids is None or len(data_ids.get('atoms', [])) == 0:
            return None
        D, M = paths.DM
        if self._cache.get('data_ids_atoms') is None:
            self._cache['data_ids_atoms'] = []
            self._cache['data'] = {'X': np.zeros((D, M, 0), dtype=float),
                                   'V': np.zeros(0, dtype=float),
                                   'F': np.zeros((D, M, 0), dtype=float)}
        if self._cache.get('data_ids_atoms') == data_ids['atoms']:
            return self._cache['data']
        else:
            new_data_ids_atoms = []
            for id in data_ids['atoms']:
                if id not in self._cache['data_ids_atoms']:
                    new_data_ids_atoms.append(id)
        atomdata = paths.atomsdata
        name2idx = atomdata.name2idx
        n2i = name2idx['atoms']
        new_data = atomdata.read({'atoms': new_data_ids_atoms})['atoms']
        P = len(new_data_ids_atoms)
        data = self._cache['data']
        if 'positions' in keys:
            positions = np.zeros((P, M, D), dtype=float)
            for i in range(P):
                positions[i] = new_data[i][n2i['positions']]
            data['X'] = np.concatenate([data['X'], positions.T], axis=2)
        ...
        if 'forces' in keys:
            forces = np.zeros((P, M, D), dtype=float)
            for i in range(P):
                forces[i] = new_data[i][n2i['forces']]
            data['F'] = np.concatenate([data['F'], forces.T], axis=2)
        self._cache['data_ids_atoms'].extend(new_data_ids_atoms)
        return data
...
class AtomicGaussian(Model):
    ...
    def read_data(self, paths, idx=None, symbol=None):
        ...
        table = 'sbdesc'
        paths.atomsdata.query(table=table, search_word='symbols=%s' % symbol)
        Xn = None
        Y = None
        return Xn, Y
...
class GaussianSearch(PathFinder):
    ...
    def __getattr__(self, key):
        if key in self.__dict__.get('real_finder', {}).finder_parameters.keys():
            ...
            raise AttributeError('No key called %s exists' % key)
    ...
    def check_maximum_energy_convergence(self, paths, idx=None, **kwargs):
        ...
        atomsdata = paths.get_data(data_ids=data_ids)
        self._muErr = np.abs(self._E_max - atomsdata['V'][-1])
    ...
    def check_alternate_saddle_convergence(self, paths, idx=None, **kwargs):
        ...
        data_ids = paths.add_data(index=idx)
        atomsdata = paths.get_data(data_ids=data_ids)
        if np.abs(E[idx] - atomsdata['V'][-1]) < self.gptol:
            return 1
        return 0
    ...
    def check_convergence(self, paths, phase=None, iter=None, logfile=None,
        ...
        if phase is None:
            phase = self.phase
        atomsdata = paths.get_data()
        if len(atomsdata['V']) < 3:
    ...
    def search(self, paths, gptol=0.01, maxiter=50, **kwargs):
        if self._pbs is True:
            _search = self._search_nuri
        elif self._pbs == 'fifi':
            _search = self._search_fifi
        elif self._pbs == 'wood':
            _search = self._search_wood
        else:
            _search = self._search
        new_paths = _search(paths, gptol=gptol, maxiter=maxiter, **kwargs)
        return new_paths.copy()

    def _search(self, paths, gptol=0.01, maxiter=50, **search_kwargs):
        label = paths.label
        if self.log is None:
            log = label + '.log'
        else:
        ...
        if os.path.exists(label + '.csv'):
            ...
            df = self.pd.read_csv(label + '.csv')
            iter_number = len(df.index) - 1
            paths = row2paths(dataframe2dct(df, index=iter_number))
        else:
            filename = label + '_initial'
            logfile.write("Writting %s \n" % (label + '.csv'))
            self._save(paths, filename=filename, mode='w', header=True)
            iter_number = 1
        ...
        while not self.check_convergence(paths, iter=i, logfile=logfile):
            ...
            logfile.write("Total data   : %d \n" % len(atomsdata['V']))
            filename = label + '_{i:02d}'.format(i=i)
            self.I_prepared_my_paths_in_various_ways(paths, logfile=logfile)
            self.real_finder.search(paths, logfile=logfile, **search_kwargs)
            self.results.update(self.real_finder.results)
            self._save(paths, filename=filename, index=[i])
            i += 1
            if self.maxtrial < i:
    ...
    def _search_nuri(self, paths, gptol=0.01, maxiter=50, **search_kwargs):
        module = "module purge\nmodule load intel/18.0.3 impi/18.0.3\n"
        qsub = ["qsub", "-V", "-N", paths.prefix, "-q", "normal", "-A", 'vasp',
        ...
        if self._write_pbs_only:
            return paths
        new_paths = self._read(paths)
        return new_paths

    def _search_wood(self, paths, gptol=0.01, maxiter=50, **search_kwargs):
        module = ''
        qsub = ["qsub", "-N", paths.prefix, "-l", "nodes=1:ppn=1", "-l",
                self._pbs_walltime]
        self._write(paths)
        self._qsub(paths, module=module, qsub=qsub)
        if self._write_pbs_only:
    ...
    def _search_fifi(self, paths, gptol=0.01, maxiter=50, **search_kwargs):
        module = ''
    ...
    def _read(self, paths):
        filename = paths.label + '_result'
        return read_csv(filename)

    def _save(self, paths, filename=None, mode='a', header=False, index=[0]):
        old_optimized = paths.model.optimized
        paths.model.optimized = False
        paths.plot(filename=filename, savefig=True, gaussian=True)
        paths.to_csv(save_calc=True, save_model=True, save_finder=True,
                     save_prj=True, save_atomsdata=True, save_plotter=True,
                     mode=mode, header=header, index=index)
        paths.model.optimized = old_optimized

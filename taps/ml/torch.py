...
class TensorProjector:
    ...
    """
    def x(self, coords):
        return tc.from_numpy(coords.coords.T)

    def _x(self, coords):
        return tc.from_numpy(coords.T)
    ...
    def V(self, potential):
        return tc.from_numpy(potential)
...
def device(cls):
    def newcls(*args, device='cpu', **kwargs):
        return cls(*args, **kwargs).to(device=device)
    return newcls

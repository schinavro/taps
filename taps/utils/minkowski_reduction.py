...
np.array = np.tensor
np.Tensor.copy = np.Tensor.clone
...
class CycleChecker:
    ...
    def __init__(self, d):
        ...
        cycles = np.array([n - i for i in range(d)]).long()
        max_cycle_length = np.prod(cycles) * d
...
def reduction_gauss(B, hu, hv):
    ...
    for it in range(MAX_IT):
        x = int(round(float(np.dot(u, v) / np.dot(u, u))))
        hu, hv = hv - x * hu, hu
        u = hu.double() @ B
        v = hv.double() @ B
        site = np.vstack([hu, hv])
...
def closest_vector(t0, u, v):
    ...
    for it in range(MAX_IT):
        ...
        dprev = ds[index]
        r = rs[index]
        kopt = int(round(float(-np.dot(t, r) / np.dot(r, r))))
        a += kopt * cs[index]
        t = t0 + a[0] * u + a[1] * v
...
def reduction_full(B):
    ...
    for it in range(MAX_IT):
        ...
        pu, pv, pw = R @ np.vstack([X, Y]).T
        nb = closest_vector(pw, pu, pv)

        # Update basis
        H[2] = np.array([nb[0], nb[1], 1]).long() @ H
        R = H.double() @ B
...
def minkowski_reduce(cell, pbc=True):
    ...
    if dim == 2:
        ...
        # maintain cell handedness
        handness1 = int(np.sign(np.linalg.det(cell)))
        handness2 = int(np.sign(np.linalg.det(op.double() @ cell)))
        if handness1 != handness2:
            op = -op

    norms1 = np.sort(np.linalg.norm(cell, axis=1))[0]
